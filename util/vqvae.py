...
class VQVAE(nn.Module):
    ...
    def img_to_reconstructed_img(self, x, v_patch_nums: Optional[Sequence[Union[int, Tuple[int, int]]]] = None, last_one=False) -> List[torch.Tensor]:
        ...
        if last_one:
            ...
            # 需要确保输出的每个张量有相同的形状
            output = [self.decoder(self.post_quant_conv(f_hat)).clamp_(-1, 1) for f_hat in ls_f_hat_BChw]
        
            # 确保所有张量的形状相同
            for i in range(1, len(output)):
                if output[i].shape != output[0].shape:
                    print(f"Shape mismatch: {output[i].shape} != {output[0].shape}")
                    # 可以通过 reshape 或其他方式调整形状
                    output[i] = torch.nn.functional.interpolate(output[i], size=output[0].shape[2:], mode='bilinear', align_corners=False)
